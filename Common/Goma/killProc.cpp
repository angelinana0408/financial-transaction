...
//  for Siluet, 
//  To the victory of Open Source!!!!

#include <windows.h>
#include <tlhelp32.h>
#include <iostream.h>

int killProcByName(const char *);
...
int main(int argc,char *argv[])
{
	...
	if (argc < 2)
	{
		cout << "USAGE: killProc ProcessName";
		exit(1);
	}

	for (int i = 1; i < argc; i++)								// 2004.06.18
	{
		memset(szName, 0x00, sizeof(szName));
		memcpy(szName, argv[i], strlen(argv[i]));
		iRes = killProcByName(szName);
	}
