...
//	LOADING STEP 1
/////////////////////////////////////////////////////////////////////////////
	RegSetInt(_REGKEY_DEVICE, _REGKEY_DEVICE_LOADINGSTEP, 1);	// 2005.04.19
/////////////////////////////////////////////////////////////////////////////
	// OPEN SHAREDATA
	OpenShareData();											// 2005.10.07

	// AP/SP UPDATE CHECK & UPDATES
	AtmUpdate();

	// OKI BRM Folder : for MM8000T
	if (IsFileExist(_DEFAULT_OKIPATH))							// 2008.12.09
	{
		WinExec("C:\\ATM\\DRVSP\\EXE\\DIPL.EXE", SW_HIDE);
		WinExecWaitEnd("C:\\ATM\\DRVSP\\EXE\\DIPLWAIT.EXE", SW_HIDE);
	}

	// FTPD START
	WinExec("C:\\T1ATM\\NETTERM\\NETFTPD.EXE", SW_HIDE);

	// REGDLL START
	WinExecWaitEnd("C:\\T1ATM\\APP\\EXERELE\\REGDLL.BAT", SW_HIDE);
	
	// RUN REAR OP
	WinExec("C:\\T1ATM\\APP\\EXERELE\\REAR.EXE", SW_HIDE);
...
/////////////////////////////////////////////////////////////////////////////
	...
	// FRONTAP START
	CString strStart;
	strStart.Format("FILE://%s\\%s", _DEF_EXERELEPATH, "T1ATM.HTM");
	m_HtmlView.Navigate(strStart);

    // Agent Start
	if(IniGetInt("D:\\INI\\TRANSACTION.INI", "TRANSACTION","AGENTOPEN", 0) == 1)
        ShellExecute(NULL, "open", "C:\\ATMS\\Agent.exe", NULL, NULL, SW_HIDE);
...
//	if (MaxMonitorIndex >= 1)									// Support 2Monitor Machine
	...
	{															// 2005.04.19
		rect.left = 800 - 4;									// 2005.04.19
		rect.top = 0 - 50;										// Title Bar Move
		rect.right = 1600 + 4;									// 2005.04.19
		rect.bottom = 600 + 100;
	}
	else
	{
...
/////////////////////////////////////////////////////////////////////////////
void CLoaderDlg::InitializeDisplayDevice()
{
	HDC		hDc;
...
/////////////////////////////////////////////////////////////////////////////
int CLoaderDlg::AtmUpdate()
{
	CString strCMD("");
	CString strFileName("");

	_mkdir(_DEF_UPDATEPATH);
	_mkdir(_DEF_APUPDATEPATH);

    IniSetStr("C:\\ATMS\\FtpDownInfo.ini","COMMON_INFO","DOWNPATH",_DEF_APUPDATEPATH);

	strCMD.Format("%s\\%s %s\\%s", 	_DEF_EXERELEPATH,
									"pkzip25 -ext -directories=full -overwrite=all -nofix",
									_DEF_UPDATEPATH,
									"V*.zip c:\\");
...
/////////////////////////////////////////////////////////////////////////////
int CLoaderDlg::IsFileExist(LPCTSTR path)						// 2008.04.10
{
    CFileStatus fs;
    if (CFile::GetStatus(path, fs) == TRUE)
        return TRUE;    
    else 
        return FALSE;    
}
...
void CLoaderDlg::OnTimer(UINT nIDEvent) 
{
	...
	if (GetShareData("OP_MODE").CompareNoCase("AP") == 0)		// 204.06.24
	{															
		HWND hRearWnd = ::FindWindow(NULL, "Rear");				// 2005.04.19
